class Color {
  ...
  Color.fromRGBO(int r, int g, int b, int opacity)
...
bool isCssColor(String color) {
  color = color.trim();
  const chNumExpr = r'-?[0-9]{1,3}(\.[0-9]+)?';
  const opNumExpr = r'-?([01]+(\.[0-9]+)?|\.[0-9]+)';
...
Color _rgbToColor(String color) {
  final channels = _parseChannels(color)!;
