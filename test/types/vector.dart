...
void main() {
  group('Vector', () {
    ...
    //   expect(await run('vector = <1, 2, 3>; vector 0'), '1');

    //   expect(await run('<>(0)'), 'IndexError: Index 0 out of range for length 0');
    //   expect(await run('<1>(5)'), 'IndexError: Index 5 out of range for length 1');
    //   expect(await run('<2> (1,2)'), 'ArgumentError: Expected 1 arguments, got 2');
    //   expect(await run('<2> "e"'), 'TypeError: Expected number, got string');
    //   expect(await run('<2> 1.5'), 'NumberError: Number is not an integer');
    // });
    test('Type', () async {
      expect(await run('type(<>)'), '"vector"');
      expect(await run('type(<1>)'), '"vector"');
      expect(await run('type(<1, 2>)'), '"vector"');
    });
    test('Length', () async {
    ...
    //   expect(await run('a = <1, 2>; b = <3, 4>; <...a, 5, ...b>'), '<1, 2, 5, 3, 4>');
